...
public class Zombie extends GameObject {
    ...
    public Zombie() {
        ...
        for (int i =0; i <= 16; i++){
            moveFrames[i]= TextureLoader.load("Assets/Textures/zombie/skeleton-move_"+i+".png", 1,1);
        }
        for (int i =0; i <= 16; i++){
            idleFrames[i]= TextureLoader.load("Assets/Textures/zombie/skeleton-idle_"+i+".png", 1,1);
        }

        for (int i =0; i <= 8; i++){
            attackFrames[i]= TextureLoader.load("Assets/Textures/zombie/skeleton-attack_"+i+".png", 1,1);
        }
        ...
        for (int i = 1; i < 19; i++){
            Audio.loadSound("zombie"+i,"Assets/Audio/zombies/new/zombie-"+i+".wav");
        }
    ...
    public void simulate(){
        ...
        target = Game.getGameInstance().player;
        float dist = (float)Math.hypot(x-target.x,y-target.y);

        for (GameObject tempTarget : Game.getGameInstance().gameObjects){
        ...
        if (attackTimer > attackCooldown){
            ...
            List<GameObject> collidingPlayer = Game.getGameInstance().collisionManager.getCollidersWithType(this,AbstractPlayer.class);
            if (collidingPlayer.size()> 0 && collidingPlayer.contains(target)){
                ((AbstractPlayer)target).hurt(damage);
            }


            List<GameObject> collidingTurret = Game.getGameInstance().collisionManager.getCollidersWithType(this,Turret.class);
        ...
        if (health <= 0){
            ...
            Game.getGameInstance().addObject(bloodSplatter);
