...
public class TankPlayer extends AbstractPlayer {
    ...
    public TankPlayer() {
        tankTexture = TextureLoader.load("Assets/Textures/tanks/KV-2_strip2.png", 2,1);
        width = 13.2f * 1.5f;
        height = 25.4f * 1.5f;
        radius = 13;
        ...
        Audio.loadSound("engine","Assets/Audio/machines/aircompressor.wav");

        Audio.loadSound("turret","Assets/Audio/machines/electric.wav");
    }
    ...
    public void simulate(){
        ...
        if (Game.getGameInstance().player != this){
            if (Input.getKeyPress(com.badlogic.gdx.Input.Keys.E)){
                score = Game.getGameInstance().player.score;
                health = Game.getGameInstance().player.health;
                ammo = Game.getGameInstance().player.ammo;
                Game.getGameInstance().player.kill();
                Game.getGameInstance().player = this;
            ...
            if (Input.getKeyPress(com.badlogic.gdx.Input.Keys.E)){
                ...
                Game.getGameInstance().addObject(playerObj);
                Game.getGameInstance().player = playerObj;
            ...
            if (state == 1){
                ...
                if (tankTreadTimer > 5){
                    ...
                    Game.getGameInstance().addObject(leftTread);
                    Game.getGameInstance().addObject(rightTread);
                    tankTreadTimer = 0;
                }
            }
            ...
            if (shootCooldownCounter > shootCooldownNumFrames & Input.getMouseClicked() == 1) {
                ...
                Game.getGameInstance().addObject(new RifleBullet(bulletX, bulletY, turretAngle-90-10));
                Game.getGameInstance().addObject(new RifleBullet(bulletX, bulletY, turretAngle-90-5));
                Game.getGameInstance().addObject(new RifleBullet(bulletX, bulletY, turretAngle-90));
                Game.getGameInstance().addObject(new RifleBullet(bulletX, bulletY, turretAngle-90+5));
                Game.getGameInstance().addObject(new RifleBullet(bulletX, bulletY, turretAngle-90+10));
