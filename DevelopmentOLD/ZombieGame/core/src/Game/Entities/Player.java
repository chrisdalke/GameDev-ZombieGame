...
public class Player extends AbstractPlayer {
    ...
    public Player() {
        playerTex = TextureLoader.load("Resources/Textures/player/shotgun/idle/survivor-idle_shotgun_0.png", 1,1);

        width = 10f;
        height = 10f;
        radius = 4;

        Audio.loadSound("bullet1","Resources/Audio/shots/pistol.wav");
        Audio.loadSound("coin","Resources/Audio/inventory/caching.wav");
    }
    ...
    public void simulate(){
        ...
        if (weapon == 0) {
            ...
            if (shootCooldownCounter > shootCooldownNumFrames & Input.getMouseClicked() == 1 & ammo > 0) {
                ...
                Game.addObject(new RifleBullet(x, y, angle));
            ...
            if (shootCooldownCounter > 2 & Input.getMouseClicked() == 1) {
                shootCooldownCounter = 0;
                for (int i = 0; i < 6; i++) {
                    Game.addObject(new FlamethrowerBullet(x, y, angle + ((float) Math.random() * 10.0f - 5.0f)));
                }
            }
        }
    }
